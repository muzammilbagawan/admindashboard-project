  ...
  useEffect(() => {
    ...
    const fetchEmailsSent = async () => {
      try {
        ...
        setEmailsSent(data.data.length); 
      } catch (error) {
    ...
    const fetchTotalSales = async () => {
      try {
        ...
        const totalCost = invoices.reduce((acc, invoice) => {
          const cost = parseFloat(invoice.cost);
          return acc + (isNaN(cost) ? 0 : cost);
        }, 0);

        setTotalSales(totalCost);
      } catch (error) {
    ...
    const fetchNewClients = async () => {
      try {
        ...
        const data = await response.json();
        setNewClients(data.data.length);
      } catch (error) {
  ...
  return (
    <Box m="20px">
      ...
      >
        ...
        >
          <StatBox
            title={`$${Number(totalSales).toFixed(2)}`}
        ...
        >
          ...
          {recentTransactions.map((transaction, i) => (
            ...
            >
              ...
              <Box color={colors.grey[100]}>{transaction.invoice_date}</Box>
              <Box
        ...
        >
          ...
          >
            ...
            <Typography
              ...
              sx={{ mt: "15px" }}
            >
              ${totalSales.toFixed(2)} revenue generated
            </Typography>
